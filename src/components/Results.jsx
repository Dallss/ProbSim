...
function Results({ appState, updateAppState }) {
  ...
  const downloadResults = () => {
    ...
    saveAs(blob, `sampling_results_${results.method}_${new Date().toISOString().split('T')[0]}.csv`)
  }
  ...
  if (error) {
    return (
      <div className="text-center py-12">
        ...
        <button
          onClick={performSamplingAndUpdate}
  ...
  const displayData = showAllData ? results.sampledData : results.sampledData.slice(0, 10)
    ...
    <div>
      ...
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-6 mb-6 border border-blue-200">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
          <div>
            <div className="text-2xl font-bold text-blue-600">{results.populationSize}</div>
            <div className="text-sm text-blue-800">Population Size</div>
          </div>
          <div>
            <div className="text-2xl font-bold text-indigo-600">{results.sampleSize}</div>
            <div className="text-sm text-indigo-800">Sample Size</div>
          </div>
          <div>
            <div className="text-2xl font-bold text-purple-600">
              {((results.sampleSize / results.populationSize) * 100).toFixed(1)}%
            </div>
            ...
            {displayData.map((row, rowIndex) => (
              ...
              >
                ...
                {columns.map((col, colIndex) => {
                  ...
                  let cellClasses = "border border-gray-300 px-3 py-2 text-sm text-gray-900"
                  
                  // Add color coding for strata and clusters
                  if (col === results.parameters.strataColumn && results.parameters.strataColumn) {
                    const stratumColor = getStratumColor(value)
                    cellClasses += ` ${stratumColor} border-2 font-medium`
                  } else if (col === results.parameters.clusterColumn && results.parameters.clusterColumn) {
                    const clusterColor = getClusterColor(value)
                    cellClasses += ` ${clusterColor} border-2 font-medium`
                  }
        ...
        <div className="mt-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
          ...
          <div className="flex flex-wrap gap-4">
            {results.parameters.strataColumn && (
              <div>
                <h4 className="font-medium text-gray-700 mb-2">Strata ({results.parameters.strataColumn})</h4>
                <div className="flex flex-wrap gap-2">
                  {Array.from(new Set(results.sampledData.map(item => item[results.parameters.strataColumn]))).map(stratum => (
                    <span
                      key={stratum}
                      className={`px-2 py-1 rounded text-xs font-medium border ${getStratumColor(stratum)}`}
                    >
                      {stratum}
                    </span>
                  ))}
            ...
            {results.parameters.clusterColumn && (
              <div>
                <h4 className="font-medium text-gray-700 mb-2">Clusters ({results.parameters.clusterColumn})</h4>
                <div className="flex flex-wrap gap-2">
                  {Array.from(new Set(results.sampledData.map(item => item[results.parameters.clusterColumn]))).map(cluster => (
                    <span
                      key={cluster}
                      className={`px-2 py-1 rounded text-xs font-medium border ${getClusterColor(cluster)}`}
                    >
                      {cluster}
                    </span>
                  ))}
      ...
      <div className="mt-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
        <h3 className="text-lg font-semibold text-blue-900 mb-3">Sampling Method Details</h3>
